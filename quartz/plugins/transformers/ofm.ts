...
function isObsidianTagLink(node: PhrasingContent): node is Link {
  if (node.type !== "link") return false
  const classNames = node.data?.hProperties?.className
  if (Array.isArray(classNames) && classNames.includes("tag-link")) return true
  return typeof node.url === "string" && /\/tags\/[^/]+$/.test(node.url)
}

/** Drop blocks that are only #tag links so they are not duplicated under TagList. */
function stripTagOnlyBlocks(tree: Root) {
  const blockIsOnlyTags = (node: Paragraph | Heading) => {
    if (!node.children.length) return false
    let hasTag = false
    for (const child of node.children) {
      if (child.type === "text") {
        if (child.value.trim() !== "") return false
      } else if (isObsidianTagLink(child)) {
        hasTag = true
      } else {
        return false
      }
  ...
  visit(tree, "paragraph", (node, index, parent) => {
    if (parent && typeof index === "number" && blockIsOnlyTags(node as Paragraph)) {
      parent.children.splice(index, 1)
      return index
    }
  })
  visit(tree, "heading", (node, index, parent) => {
    if (parent && typeof index === "number" && blockIsOnlyTags(node as Heading)) {
      parent.children.splice(index, 1)
      return index
    }
  })
...
/** Remove leading #tag links from content blocks to avoid duplicate pills with TagList. */
function stripLeadingTagLinks(tree: Root) {
  const cleanup = (node: Paragraph | Heading) => {
    if (!node.children.length) return false
    let changed = false
    ...
    // Normalize leftover leading whitespace after removed tag chips.
    const first = node.children[0]
    if (first?.type === "text") {
    ...
    return changed && node.children.length === 0
  }

  visit(tree, "paragraph", (node, index, parent) => {
    if (parent && typeof index === "number" && cleanup(node as Paragraph)) {
      parent.children.splice(index, 1)
      return index
    }
  })
  visit(tree, "heading", (node, index, parent) => {
    if (parent && typeof index === "number" && cleanup(node as Heading)) {
      parent.children.splice(index, 1)
      return index
    }
  })
    ...
    markdownPlugins(ctx) {
      ...
      plugins.push(() => {
        return (tree: Root, file) => {
          ...
          if (opts.parseTags) {
            stripTagOnlyBlocks(tree)
            stripLeadingTagLinks(tree)
          }
        }
      })
